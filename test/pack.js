...
describe('GraphicsKitPublisher packs project', function () {
  ...
  it('Should error if homepage in package.json is wrong hostname', async function () {
    ...
    try {
      ...
      expect(e.message).to.contain(
        chalk`Invalid {yellow homepage} in {cyan package.json}`
      );
    }
  });
  ...
  it('Should set homepage from package.json', async function () {
    ...
    expect(homepage).to.be('https://www.reuters.com/graphics/project/');
  });

  it('Should create public edition from dist', async function () {
    const graphicsPublisher = new GraphicsPublisher();
    await graphicsPublisher.makePublicEdition();
    expect(fs.existsSync('graphics-pack/public/interactive/index.html')).to.be(
      true
    );
    expect(
  ...
  it('Should create static asset editions', async function () {
    ...
    expect(fs.existsSync('graphics-pack/media-en-chart/JPG/chart.jpg')).to.be(
      true
    );
    expect(fs.existsSync('graphics-pack/media-en-chart/EPS/chart.eps')).to.be(
      true
    );
    expect(fs.existsSync('graphics-pack/media-en-map/JPG/my-map.jpg')).to.be(
      true
    );
    expect(fs.existsSync('graphics-pack/media-de-chart/JPG/chart.jpg')).to.be(
      true
    );
  });
