...
Locale codes are any valid language code like `en`, `de`, `es`.

A slug is a valid slug ID used to identify the archive and the graphic it contains, like `map`, `bar-chart` or `page`.
...
<Aside type="caution">
The publisher expects the root `index.html` file for all interactive editions will have a [canonical link element](https://en.wikipedia.org/wiki/Canonical_link_element) that specifies the published location of the page. Make sure one is added to the `head` section of those pages.
