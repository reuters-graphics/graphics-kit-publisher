...
<FileTree>
- media-en-map <small>Archive</small>
  - interactive <small>Edition</small>
    - **index.html** <small>Root file</small>
    - cdn <small>This archive's own copy of the app's assets</small>
      - styles.css
      - scripts.js
...
<Aside title="Each archive carries its own assets">
Every archive includes its own copy of the app's assets and points at them at its own URL, so no archive depends on any other. That's what lets you [upload one archive without touching the rest](/graphics-kit-publisher/commands/#upload).

It also means only the embed's own page is in the archive — so an embed shouldn't link to other pages in your project.
</Aside>
...
<Aside type="caution">
Only [interactive editions](#interactive-editions) should be made searchable in Lynx.
