...
Each branch previews to its own subdirectory, so two people previewing at once don't overwrite each other:

```
https://graphics.thomsonreuters.com/testfiles/2025/ayzrxlqerve/_branches/feat-new-map/
```

`main` publishes to your preview URL itself, so the link you've shared for the project keeps working. See [preview config](/graphics-kit-publisher/config/preview/) to change which branches do that, or to turn per-branch previews off.

To publish under a different name -- or to the preview URL itself, whatever branch you're on:

<Tabs>
  <TabItem label="CLI" icon="setting">
    ```console
    pnpm graphics-publisher preview --branch my-other-name
    pnpm graphics-publisher preview --no-branch
    ```
  </TabItem>
  <TabItem label="Graphics kit" icon="laptop">
    ```console
    pnpm preview --branch my-other-name
    pnpm preview --no-branch
    ```
  </TabItem>
</Tabs>
