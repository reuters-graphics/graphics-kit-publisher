...
Uploading a subset is worth it on packs with lots of embeds: the graphics server processes archives one at a time, taking a few minutes each, so re-uploading nine embeds to fix one of them is nine times the wait. The archives you skip are left untouched and go on serving — each archive carries [its own copy of everything it needs](/graphics-kit-publisher/sphinx/#interactive-editions).
