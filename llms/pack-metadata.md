...
1. **Sourced before upload** — values the **Sphinx server requires** to create or update a pack (title, slugs, authors, desk, description, contactEmail, …). Before uploading, the publisher fills each from a [metadata pointer](#sourcing-metadata-pointers) if one resolves, and otherwise **prompts the human running the CLI** to type it in — all of it in a single phase, before anything is uploaded. ✅ **You MAY write or edit this metadata.** Writing these values into `package.json` yourself satisfies the server's requirement up front, so the publisher already has them and the upload doesn't stall on interactive prompts an agent can't answer. Changes propagate to the server pack on the next upload.
