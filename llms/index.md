...
1. `graphics-publisher preview` — Build and upload a preview to the testfiles S3 bucket (no graphics server pack created). Each branch gets its own subdirectory, so concurrent previews don't overwrite each other; `main` publishes to the project's preview URL itself. Override with `--branch <name>` or `--no-branch` (see [`preview` config](./config.md#preview)).
