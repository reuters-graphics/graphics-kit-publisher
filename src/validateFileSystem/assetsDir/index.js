...
export default {
  ...
  validateAssetsDir(ASSETS_DIR) {
    if (!fs.existsSync(ASSETS_DIR)) throw new FileSystemError(chalk`Media assets directory {yellow ${path.relative(this.CWD, ASSETS_DIR)}} does not exist. It needs to.`);
    ...
    for (const embedLocaleDir of embedLocalesDirs) {
      ...
      if (!VALID_LOCALES.includes(locale)) throw new FileSystemError(chalk`Invalid directory in media assets: {cyan ${locale}}. All directories beneath {yellow ${path.relative(this.CWD, ASSETS_DIR)}} should be named with a valid locale code.`);
      ...
      for (const embed of embeds) {
        ...
        if (jpgs.length < 1) throw new FileSystemError(chalk`Did not find a {cyan .jpg} image in {yellow ${path.relative(this.CWD, EMBED_DIR)}}. One is required.`);
      }
    }
  },
};
