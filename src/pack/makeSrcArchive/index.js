...
export default {
  async makeSrcArchive() {
    ...
    const ARCHIVE_TEMP_PATH = path.join(this.PACK_DIR, '_app.zip');
    const ARCHIVE_TEMP_RELATIVE_PATH = path.relative(PKG_ROOT, ARCHIVE_TEMP_PATH);
    const ARCHIVE_DIR_PATH = path.join(this.PACK_DIR, 'app');
    const ASSETS_DIR = path.relative(PKG_ROOT, this.ASSETS_DIR);
    spawnSync('git', ['archive', '-o', ARCHIVE_TEMP_RELATIVE_PATH, 'HEAD', '.', `:!${ASSETS_DIR}`, ':!project-files'], { cwd: PKG_ROOT });

    // Re-zip the archive for:
    // https://github.com/reuters-graphics/graphics-kit-publisher/issues/52
    await new Promise((resolve) => {
      fs.createReadStream(ARCHIVE_TEMP_PATH)
        .pipe(
          unzipper.Extract({ path: ARCHIVE_DIR_PATH })
            .on('close', async() => {
